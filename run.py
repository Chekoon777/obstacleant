...
def train():
    ...
    """
    TIMESTEPS = 25000
    model.learn(total_timesteps=int(TIMESTEPS), tb_log_name=f"{args.sb3_algo}", callback=eval_callback)

def test():        
    model = sb3_class.load(os.path.join(model_dir, f"{args.sb3_algo}", "best_model"), env=env)
...
if __name__ == '__main__':
    ...
    if args.test:
        env = CustomAntEnv(render_mode='human')
        test()
    else:
        env = CustomAntEnv()
        env = Monitor(env)
